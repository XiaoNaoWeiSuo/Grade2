...
> 本文件以 2026-09 第二次重建（爬虫内核驱动）+ 2026-09-11 正式 UI 落地为准，适用于 `2.5.31+1`。
> UI 设计基准另见 [design-spec.md](design-spec.md)。
...
| 层 | 选型 |
|---|---|
| 语言/平台 | Dart + Flutter（`sdk: >=3.11.0 <4.0.0`），UI 全 Cupertino（iOS 原生风格） |
| 架构 | 严格 MVVM + Riverpod 2.6 |
| 依赖 | `flutter_riverpod` + `path_provider` + `flutter_secure_storage` + `cupertino_icons` + `flutter_localizations`（**内核零第三方依赖**） |
...
| 国际化 | 自建词典 `AppStrings`：简体中文 / 繁體中文 / English / 日本語 / اردو |
| 主题 | `AppThemeMode`：system / light / dark / eye（护眼），`AppPalette` 语义色板 |

依赖最小化原则见 [pubspec.yaml](../pubspec.yaml)：内核零第三方；UI 层仅保留状态管理、
路径定位、密码安全存储（Keychain/Keystore）、Cupertino 图标字体与框架本地化委托，
其余（intl/dio/html/crypto/email/url_launcher 等）已全部裁除。

## 3. 分层总览

```text
lib/
├── main.dart                    入口：CupertinoApp + locale/delegates + 主题色板 + 按认证状态路由
├── l10n/                        本地化层（零业务逻辑）
│   ├── app_strings.dart         5 语言词典 + AppLocale + context.l10n 扩展
│   ├── app_theme.dart           4 主题模式 + AppPalette 色板 + AppThemeScope/buildTheme
│   └── app_settings.dart        语言/主题设置 Notifier（LocalCache ns=meta 持久化）
...
└── views/                       表现层（Cupertino 页面 + 共享组件，可整体替换不影响内核）
```
...
| `secureStorageProvider` | Provider | FlutterSecureStorage（账号簿密码 Keychain/Keystore） |
...
| `appSettingsProvider` | Notifier | 语言 + 主题模式（ns=meta 持久化，启动 load） |
| `semesterSelectionProvider` | AsyncNotifier | 全局学期选择单一数据源 |
| `timetableProvider` | AsyncNotifier | 课程表（缓存优先 + 强制刷新 + 周次翻页） |
| `gradesProvider` | AsyncNotifier | 成绩（缓存优先 + 跟随全局学期） |
| `examProvider` | AsyncNotifier | 考试（批次/安排/课外/中期四子 AsyncValue 独立渲染） |
| `planProvider` | AsyncNotifier | 培养计划（完成度/我的计划/培养方案/转专业并行拉取） |
| `electiveProvider` | AsyncNotifier | 选课跨接口编排（轮次→上下文→课程→余量合并 + myElect/ttCodes 已选判定） |
| `evaluateProvider` | AsyncNotifier | 教学评价（缓存优先） |
| `messagesProvider` | AsyncNotifier | 系统消息（缓存优先） |
| `stdDetailProvider` | AsyncNotifier | 学籍详情（分段 KV + 照片） |
| `welcomeProvider` | AsyncNotifier | 教务首页公告/新闻模块 |
| `weekIndexProvider` | StateProvider | 当前教学周（翻页不触发网络） |

关键数据流：
```text
main.initState → appSettingsProvider.load()（读 ns=meta 的 theme/locale，经 watch 重建 CupertinoApp）
...
electiveProvider.openProfile → electiveContext → electiveLessons → electiveCounts 合并余量
  → 已选 = myElect(ns=elective, my_elect_<profileId>) ∪ ttCodes(当前学期课表课程号)
```
...
## 6. 表现层（views，Cupertino 单栈）

信息架构：**无底部 TabBar**，课表即常驻首页；其余能力经导航栏
leading 头像 →『账户』、trailing →『更多』中枢下钻（设计基准见 [design-spec.md](design-spec.md)）。

```text
lib/views/
├── widgets/cupertino_kit.dart   共享组件（Group/SectionHeader/CupertinoEmpty/AsyncSliver/
│                                toast/confirm/JSON 弹窗/pushPage…）
└── pages/
    ├── load_page.dart           Splash
    ├── login_page.dart          登录 + 短信二次认证面板
    ├── timetable_page.dart      课表首页：Table 网格 + 周翻页 + 学期弹层 + 更多/账户入口
    ├── hub_page.dart            『更多』功能中枢（窗期高频/数据查询/事务三组）
    ├── account_page.dart        『账户』：账号信息 + 外观/语言设置 + 学期同步 + 清缓存 + 登出
    ├── accounts_page.dart       账号簿多账号管理（列表/切换/删除/登记，密码走安全存储）
    ├── elective_page.dart       选课轮次列表
    ├── elective_lessons_page.dart  课程列表 + 余量徽标 + 多标签筛选/搜索 + 选/退 + 详情弹层
    ├── exam_page.dart           考试批次/安排 + 四六级 + 中期考核
    ├── grades_page.dart         成绩表（Table + KeyedSubtree 整表重建，点击行看全部键值）
    ├── plan_page.dart           培养计划（完成度/我的计划/培养方案/转专业）
    ├── std_detail_page.dart     学籍详情（分段 KV + 照片）
    ├── welcome_page.dart        公告/新闻
    ├── messages_page.dart       系统消息
    └── evaluate_page.dart       教学评价任务
```

路由（[main.dart](../lib/main.dart)）：
```text
CupertinoApp（locale/delegates/AppThemeScope 包裹）
  └ _AuthGate: authProvider.when
       loading/boot ──► Splash
       busy/needsLogin/needsSms ──► LoginPage
       authed ──► AuthedHome（嵌套 Navigator，首路由=课表；
                   下钻页 CupertinoPageRoute 压栈，登出整棵卸载）
```
...
| 账号簿密码 | 系统 Keychain / Android Keystore（FlutterSecureStorage） | key=`pwd_<username>` |

LocalCache 命名空间约定：
`auth`（账号簿元数据/最后登录，**密码不在内**）、`timetable`（`table_std_<semesterId>`）、
`meta`（`current_semester`/`semester_list`/`theme`/`locale`）、`grades`、`evaluate`、`messages`、
`welcome`、`elective`（`my_elect_<profileId>` 本地已选记录）。
> 会话状态与业务缓存**严格分离**：只存令牌 vs 只存业务数据；密码只进安全存储。
...
| [design-spec.md](design-spec.md) | 正式 UI 设计规格（IA/页面/交互基准） |
